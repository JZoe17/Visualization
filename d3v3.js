!function() {
  ...
  if (d3v3_document) {
    try {
      ...
      var d3v3_element_prototype = this.Element.prototype, d3v3_element_setAttribute = d3v3_element_prototype.setAttribute, d3v3_element_setAttributeNS = d3v3_element_prototype.setAttributeNS, d3v3_style_prototype = this.CSSStyleDeclaration.prototype, d3v3_style_setProperty = d3v3_style_prototype.setProperty;
  ...
  d3v3.transpose = function(matrix) {
    ...
    for (var i = -1, m = d3v3.min(matrix, d3v3_transposeLength), transpose = new Array(m); ++i < m; ) {
  ...
  function d3v3_transposeLength(d) {
    return d.length;
  }
  d3v3.zip = function() {
  ...
  }, d3v3_selectMatches = function(n, s) {
    var d3v3_selectMatcher = n.matches || n[d3v3_vendorSymbol(n, "matchesSelector")];
    d3v3_selectMatches = function(n, s) {
      return d3v3_selectMatcher.call(n, s);
    };
    return d3v3_selectMatches(n, s);
  };
  ...
  d3v3.selection = function() {
    return d3v3.select(d3v3_document.documentElement);
  };
  ...
  var d3v3_nsXhtml = "http://www.w3.org/1999/xhtml";
  var d3v3_nsPrefix = {
  ...
  d3v3_selectionPrototype.style = function(name, value, priority) {
    ...
    if (n < 3) {
      ...
      if (n < 2) {
        var node = this.node();
        return d3v3_window(node).getComputedStyle(node, null).getPropertyValue(name);
      }
      priority = "";
    }
  ...
  function d3v3_selection_creator(name) {
    function create() {
      ...
      return namespace === d3v3_nsXhtml && document.documentElement.namespaceURI === d3v3_nsXhtml ? document.createElement(name) : document.createElementNS(namespace, name);
    }
    function createNS() {
  ...
  d3v3.select = function(node) {
    ...
    if (typeof node === "string") {
      group = [ d3v3_select(node, d3v3_document) ];
      group.parentNode = d3v3_document.documentElement;
    } else {
      group = [ node ];
      group.parentNode = d3v3_documentElement(node);
    }
  ...
  d3v3.selectAll = function(nodes) {
    ...
    if (typeof nodes === "string") {
      group = d3v3_array(d3v3_selectAll(nodes, d3v3_document));
      group.parentNode = d3v3_document.documentElement;
    } else {
      group = d3v3_array(nodes);
  ...
  var d3v3_event_dragSelect, d3v3_event_dragId = 0;
  function d3v3_event_dragSuppress(node) {
    var name = ".dragsuppress-" + ++d3v3_event_dragId, click = "click" + name, w = d3v3.select(d3v3_window(node)).on("touchmove" + name, d3v3_eventPreventDefault).on("dragstart" + name, d3v3_eventPreventDefault).on("selectstart" + name, d3v3_eventPreventDefault);
    if (d3v3_event_dragSelect == null) {
      d3v3_event_dragSelect = "onselectstart" in node ? false : d3v3_vendorSymbol(node.style, "userSelect");
    }
    if (d3v3_event_dragSelect) {
      var style = d3v3_documentElement(node).style, select = style[d3v3_event_dragSelect];
  ...
  var d3v3_mouse_bug44083 = this.navigator && /WebKit/.test(this.navigator.userAgent) ? -1 : 0;
  function d3v3_mousePoint(container, e) {
    if (e.changedTouches) e = e.changedTouches[0];
    var svg = container.ownerSVGElement || container;
    if (svg.createSVGPoint) {
      var point = svg.createSVGPoint();
      if (d3v3_mouse_bug44083 < 0) {
        var window = d3v3_window(container);
  ...
  d3v3.behavior.drag = function() {
    var event = d3v3_eventDispatch(drag, "drag", "dragstart", "dragend"), origin = null, mousedown = dragstart(d3v3_noop, d3v3.mouse, d3v3_window, "mousemove", "mouseup"), touchstart = dragstart(d3v3_behavior_dragTouchId, d3v3.touch, d3v3_identity, "touchmove", "touchend");
    function drag() {
      this.on("mousedown.drag", mousedown).on("touchstart.drag", touchstart);
    }
    function dragstart(id, position, subject, move, end) {
      return function() {
        var that = this, target = d3v3.event.target.correspondingElement || d3v3.event.target, parent = that.parentNode, dispatch = event.of(that, arguments), dragged = 0, dragId = id(), dragName = ".drag" + (dragId == null ? "" : "-" + dragId), dragOffset, dragSubject = d3v3.select(subject(target)).on(move + dragName, moved).on(end + dragName, ended), dragRestore = d3v3_event_dragSuppress(target), position0 = position(parent, dragId);
        if (origin) {
  ...
  var ε = 1e-6, ε2 = ε * ε, π = Math.PI, τ = 2 * π, τε = τ - ε, halfπ = π / 2, d3v3_radians = π / 180, d3v3_degrees = 180 / π;
  function d3v3_sgn(x) {
  ...
  d3v3.interpolateZoom = function(p0, p1) {
    ...
    if (d2 < ε2) {
      ...
      S = (r1 - r0) / ρ;
      i = function(t) {
        var s = t * S, coshr0 = d3v3_cosh(r0), u = w0 / (ρ2 * d1) * (coshr0 * d3v3_tanh(ρ * s + r0) - d3v3_sinh(r0));
        return [ ux0 + u * dx, uy0 + u * dy, w0 * coshr0 / d3v3_cosh(ρ * s + r0) ];
      };
    }
    i.duration = S * 1e3;
    return i;
  ...
  d3v3.behavior.zoom = function() {
    ...
    if (!d3v3_behavior_zoomWheel) {
      d3v3_behavior_zoomWheel = "onwheel" in d3v3_document ? (d3v3_behavior_zoomDelta = function() {
    ...
    function mousedowned() {
      var that = this, dispatch = event.of(that, arguments), dragged = 0, subject = d3v3.select(d3v3_window(that)).on(mousemove, moved).on(mouseup, ended), location0 = location(d3v3.mouse(that)), dragRestore = d3v3_event_dragSuppress(that);
    ...
    function touchstarted() {
      var that = this, dispatch = event.of(that, arguments), locations0 = {}, distance0 = 0, scale0, zoomName = ".zoom-" + d3v3.event.changedTouches[0].identifier, touchmove = "touchmove" + zoomName, touchend = "touchend" + zoomName, targets = [], subject = d3v3.select(that), dragRestore = d3v3_event_dragSuppress(that);
      started();
      zoomstarted(dispatch);
    ...
    function mousewheeled() {
      ...
      if (mousewheelTimer) clearTimeout(mousewheelTimer); else d3v3_selection_interrupt.call(this), 
  ...
  var d3v3_behavior_zoomInfinity = [ 0, Infinity ], d3v3_behavior_zoomDelta, d3v3_behavior_zoomWheel;
  ...
  function d3v3_rgb_parse(format, rgb, hsl) {
    ...
    if (color = d3v3_rgb_names.get(format)) {
  ...
  var d3v3_timer_queueHead, d3v3_timer_queueTail, d3v3_timer_interval, d3v3_timer_timeout, d3v3_timer_frame = this[d3v3_vendorSymbol(this, "requestAnimationFrame")] || function(callback) {
    setTimeout(callback, 17);
  };
  d3v3.timer = function() {
    d3v3_timer.apply(this, arguments);
  };
  function d3v3_timer(callback, delay, then) {
    var n = arguments.length;
    if (n < 2) delay = 0;
  ...
  function d3v3_timer_mark() {
    var now = Date.now(), timer = d3v3_timer_queueHead;
    while (timer) {
  ...
  d3v3.geo.bounds = function() {
    ...
    function linePoint(λ, φ) {
      ...
      if (p0) {
        ...
        var dλ = λ - λ_, s = dλ > 0 ? 1 : -1, λi = inflection[0] * d3v3_degrees * s, antimeridian = abs(dλ) > 180;
  ...
  d3v3.geo.centroid = function(object) {
    ...
    if (m < ε2) {
      ...
      if (d3v3_geo_centroidW1 < ε) x = d3v3_geo_centroidX0, y = d3v3_geo_centroidY0, z = d3v3_geo_centroidZ0;
      m = x * x + y * y + z * z;
  ...
  function d3v3_geo_centroidPoint(λ, φ) {
    ...
    d3v3_geo_centroidPointXYZ(cosφ * Math.cos(λ), cosφ * Math.sin(λ), Math.sin(φ));
  }
  ...
  function d3v3_geo_centroidRingStart() {
    ...
    function nextPoint(λ, φ) {
      λ *= d3v3_radians;
      var cosφ = Math.cos(φ *= d3v3_radians), x = cosφ * Math.cos(λ), y = cosφ * Math.sin(λ), z = Math.sin(φ), cx = y0 * z - z0 * y, cy = z0 * x - x0 * z, cz = x0 * y - y0 * x, m = Math.sqrt(cx * cx + cy * cy + cz * cz), u = x0 * x + y0 * y + z0 * z, v = m && -d3v3_acos(u) / m, w = Math.atan2(m, u);
  ...
  var d3v3_geo_clipAntimeridian = d3v3_geo_clip(d3v3_true, d3v3_geo_clipAntimeridianLine, d3v3_geo_clipAntimeridianInterpolate, [ -π, -π / 2 ]);
  ...
  function d3v3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        d3v3_geo_areaRingSum.add(Math.atan2(k * sdλ * Math.sin(adλ), cosφ0 * cosφ + k * Math.cos(adλ)));
        ...
        if (antimeridian ^ λ0 >= meridian ^ λ >= meridian) {
          ...
          var φarc = (antimeridian ^ dλ >= 0 ? -1 : 1) * d3v3_asin(intersection[2]);
    ...
    return (polarAngle < -ε || polarAngle < ε && d3v3_geo_areaRingSum < -ε) ^ winding & 1;
  }
  function d3v3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, notHemisphere = abs(cr) > ε, interpolate = d3v3_geo_circleInterpolate(radius, 6 * d3v3_radians);
    return d3v3_geo_clip(visible, clipLine, interpolate, smallRadius ? [ 0, -radius ] : [ -π, radius - π ]);
  ...
  function d3v3_geo_resample(project) {
    var δ2 = .5, cosMinDistance = Math.cos(30 * d3v3_radians), maxDepth = 16;
  ...
  function d3v3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, λ = 0, φ = 0, δλ = 0, δφ = 0, δγ = 0, δx, δy, preclip = d3v3_geo_clipAntimeridian, postclip = d3v3_identity, clipAngle = null, clipExtent = null, stream;
  ...
  function d3v3_geo_identityRotation(λ, φ) {
    return [ λ > π ? λ - τ : λ < -π ? λ + τ : λ, φ ];
  }
  d3v3_geo_identityRotation.invert = d3v3_geo_equirectangular;
  function d3v3_geo_rotation(δλ, δφ, δγ) {
    return δλ ? δφ || δγ ? d3v3_geo_compose(d3v3_geo_rotationλ(δλ), d3v3_geo_rotationφγ(δφ, δγ)) : d3v3_geo_rotationλ(δλ) : δφ || δγ ? d3v3_geo_rotationφγ(δφ, δγ) : d3v3_geo_identityRotation;
  }
  ...
  function d3v3_geo_rotationφγ(δφ, δγ) {
    ...
    function rotation(λ, φ) {
      ...
      return [ Math.atan2(y * cosδγ - k * sinδγ, x * cosδφ - z * sinδφ), d3v3_asin(k * cosδγ + y * sinδγ) ];
    }
    rotation.invert = function(λ, φ) {
      var cosφ = Math.cos(φ), x = Math.cos(λ) * cosφ, y = Math.sin(λ) * cosφ, z = Math.sin(φ), k = z * cosδγ - y * sinδγ;
      return [ Math.atan2(y * cosδγ + z * sinδγ, x * cosδφ + k * sinδφ), d3v3_asin(k * cosδφ - x * sinδφ) ];
    };
    return rotation;
  }
  ...
  function d3v3_geo_lengthLineStart() {
    ...
    function nextPoint(λ, φ) {
      var sinφ = Math.sin(φ *= d3v3_radians), cosφ = Math.cos(φ), t = abs((λ *= d3v3_radians) - λ0), cosΔλ = Math.cos(t);
  ...
  function d3v3_geo_conicEquidistant(φ0, φ1) {
    ...
    if (abs(n) < ε) return d3v3_geo_equirectangular;
    function forward(λ, φ) {
      var ρ = G - φ;
    ...
    while (iCell--) {
      ...
      while (iHalfEdge < nHalfEdges) {
        ...
        if (abs(x3 - x2) > ε || abs(y3 - y2) > ε) {
          halfEdges.splice(iHalfEdge, 0, new d3v3_geom_voronoiHalfEdge(d3v3_geom_voronoiCreateBorderEdge(cell.site, end, abs(x3 - x0) < ε && y1 - y3 > ε ? {
            x: x0,
            y: abs(x2 - x0) < ε ? y2 : y1
          } : abs(y3 - y1) < ε && x1 - x3 > ε ? {
            x: abs(y2 - y1) < ε ? x2 : x1,
  ...
  function d3v3_geom_voronoiClipEdges(extent) {
    ...
    while (i--) {
      e = edges[i];
      if (!d3v3_geom_voronoiConnectEdge(e, extent) || !clip(e) || abs(e.a.x - e.b.x) < ε && abs(e.a.y - e.b.y) < ε) {
        e.a = e.b = null;
        edges.splice(i, 1);
      }
    }
  ...
  d3v3.interpolators = [ function(a, b) {
    var t = typeof b;
    return (t === "string" ? d3v3_rgb_names.has(b.toLowerCase()) || /^(#|rgb\(|hsl\()/i.test(b) ? d3v3_interpolateRgb : d3v3_interpolateString : b instanceof d3v3_color ? d3v3_interpolateRgb : Array.isArray(b) ? d3v3_interpolateArray : t === "object" && isNaN(b) ? d3v3_interpolateObject : d3v3_interpolateNumber)(a, b);
  } ];
  ...
  function d3v3_interpolateTransformPop(s) {
    return s.length ? s.pop() + "," : "";
  }
  function d3v3_interpolateTranslate(ta, tb, s, q) {
  ...
  function d3v3_interpolateRotate(ra, rb, s, q) {
    if (ra !== rb) {
      if (ra - rb > 180) rb += 360; else if (rb - ra > 180) ra += 360;
      q.push({
        i: s.push(d3v3_interpolateTransformPop(s) + "rotate(", null, ")") - 2,
        x: d3v3_interpolateNumber(ra, rb)
      });
    } else if (rb) {
      s.push(d3v3_interpolateTransformPop(s) + "rotate(" + rb + ")");
    }
  }
  function d3v3_interpolateSkew(wa, wb, s, q) {
    if (wa !== wb) {
      q.push({
        i: s.push(d3v3_interpolateTransformPop(s) + "skewX(", null, ")") - 2,
        x: d3v3_interpolateNumber(wa, wb)
      });
    } else if (wb) {
      s.push(d3v3_interpolateTransformPop(s) + "skewX(" + wb + ")");
    }
  }
  function d3v3_interpolateScale(ka, kb, s, q) {
    if (ka[0] !== kb[0] || ka[1] !== kb[1]) {
      var i = s.push(d3v3_interpolateTransformPop(s) + "scale(", null, ",", null, ")");
      q.push({
        i: i - 4,
      ...
      s.push(d3v3_interpolateTransformPop(s) + "scale(" + kb + ")");
    }
  }
  function d3v3_interpolateTransform(a, b) {
    var s = [], q = [];
    a = d3v3.transform(a), b = d3v3.transform(b);
    d3v3_interpolateTranslate(a.translate, b.translate, s, q);
    d3v3_interpolateRotate(a.rotate, b.rotate, s, q);
    d3v3_interpolateSkew(a.skew, b.skew, s, q);
    d3v3_interpolateScale(a.scale, b.scale, s, q);
    a = b = null;
    return function(t) {
  ...
  d3v3.layout.force = function() {
    var force = {}, event = d3v3.dispatch("start", "tick", "end"), timer, size = [ 1, 1 ], drag, alpha, friction = .9, linkDistance = d3v3_layout_forceLinkDistance, linkStrength = d3v3_layout_forceLinkStrength, charge = -30, chargeDistance2 = d3v3_layout_forceChargeDistance2, gravity = .1, theta2 = .64, nodes = [], links = [], distances, strengths, charges;
  ...
  d3v3.layout.pie = function() {
    var value = Number, sort = d3v3_layout_pieSortByValue, startAngle = 0, endAngle = τ, padAngle = 0;
    function pie(data) {
  ...
  function d3v3_scale_linearNice(domain, m) {
    d3v3_scale_nice(domain, d3v3_scale_niceStep(d3v3_scale_linearTickRange(domain, m)[2]));
    d3v3_scale_nice(domain, d3v3_scale_niceStep(d3v3_scale_linearTickRange(domain, m)[2]));
    return domain;
  }
  ...
  d3v3.svg.arc = function() {
    ...
    function arc() {
      ...
      if (da > ε && (rc = Math.min(Math.abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments))) > .001) {
        ...
        if (x1 != null) {
          var t30 = d3v3_svg_arcCornerTangents(x3 == null ? [ x2, y2 ] : [ x3, y3 ], [ x0, y0 ], r1, rc1, cw), t12 = d3v3_svg_arcCornerTangents([ x1, y1 ], [ x2, y2 ], r1, rc1, cw);
          if (rc === rc1) {
        ...
        if (x3 != null) {
          var t03 = d3v3_svg_arcCornerTangents([ x0, y0 ], [ x3, y3 ], r0, -rc0, cw), t21 = d3v3_svg_arcCornerTangents([ x2, y2 ], x1 == null ? [ x0, y0 ] : [ x1, y1 ], r0, -rc0, cw);
          if (rc === rc0) {
  ...
  function d3v3_svg_lineCardinalClosed(points, tension) {
    return points.length < 3 ? d3v3_svg_lineLinearClosed(points) : points[0] + d3v3_svg_lineHermite((points.push(points[0]), 
  ...
  d3v3.transition = function(selection, name) {
    return selection && selection.transition ? d3v3_transitionInheritId ? selection.transition(name) : selection : d3v3.selection().transition(selection);
  };
  ...
  d3v3_transitionPrototype.style = function(name, value, priority) {
    ...
    function styleString(b) {
      return b == null ? styleNull : (b += "", function() {
        var a = d3v3_window(this).getComputedStyle(this, null).getPropertyValue(name), i;
  ...
  d3v3_transitionPrototype.styleTween = function(name, tween, priority) {
    ...
    function styleTween(d, i) {
      var f = tween.call(this, d, i, d3v3_window(this).getComputedStyle(this, null).getPropertyValue(name));
      return f && function(t) {
  ...
  function d3v3_transitionNode(node, i, ns, id, inherit) {
    ...
    if (!transition) {
      time = inherit.time;
      timer = d3v3_timer(schedule, 0, time);
      transition = lock[id] = {
        tween: new d3v3_Map(),
        time: time,
        timer: timer,
  ...
  d3v3.svg.axis = function() {
    ...
    function axis(g) {
      g.each(function() {
        ...
        var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues, tickFormat = tickFormat_ == null ? scale1.tickFormat ? scale1.tickFormat.apply(scale1, tickArguments_) : d3v3_identity : tickFormat_, tick = g.selectAll(".tick").data(ticks, scale1), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick").style("opacity", ε), tickExit = d3v3.transition(tick.exit()).style("opacity", ε).remove(), tickUpdate = d3v3.transition(tick.order()).style("opacity", 1), tickSpacing = Math.max(innerTickSize, 0) + tickPadding, tickTransform;
    ...
    axis.ticks = function() {
      ...
      tickArguments_ = d3v3_array(arguments);
      return axis;
    };
  ...
  d3v3.svg.brush = function() {
    ...
    function brushstart() {
      var target = this, eventTarget = d3v3.select(d3v3.event.target), event_ = event.of(target, arguments), g = d3v3.select(target), resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x, resizingY = !/^(e|w)$/.test(resizing) && y, dragging = eventTarget.classed("extent"), dragRestore = d3v3_event_dragSuppress(target), center, origin = d3v3.mouse(target), offset;
      var w = d3v3.select(d3v3_window(target)).on("keydown.brush", keydown).on("keyup.brush", keyup);
  ...
  if (typeof define === "function" && define.amd) this.d3v3 = d3v3, define(d3v3); else if (typeof module === "object" && module.exports) module.exports = d3v3; else this.d3v3 = d3v3;
}();